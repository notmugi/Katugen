...
def ensure_gtk_css_import(gtk_css: Path, colors_file: Path, label: str) -> bool:
    """
    Append the matugen.css import to gtk.css if not already present.
    If gtk.css doesn't exist, create it with the import.
    Does not overwrite user modifications (similar to niri template).
    """
    if not colors_file.exists():
        print(f"Error: {label} matugen.css not found at {colors_file}", file=sys.stderr)
        return False
    ...
    if gtk_css.exists() or gtk_css.is_symlink():
        ...
        if "matugen.css" in content and "@import" in content:
            return True
        ...
        print(f"Appended {label} matugen.css import to gtk.css")
    else:
        gtk_css.write_text(GTK_IMPORT + "\n")
        print(f"Created {label} gtk.css with matugen.css import")
    return True


async def apply_gtk3_colors(config_dir: Path):
    gtk3_dir = config_dir / "gtk-3.0"
    colors_file = gtk3_dir / "matugen.css"
...
async def apply_gtk4_colors(config_dir: Path):
    ...
    colors_file = gtk4_dir / "matugen.css"
...
async def main():
    ...
    if all(results):
        ...
        # gtk.css / matugen.css setup failed.
        await sync_system_appearance(mode, update_gtk_theme=False)
        sys.exit(1)
